...
/**
 * leader通知Broker注册成功。
 * 1、接口不用考虑幂等性。因为BrokerService.initCURRENT_NODE()可以被重复调用
...
public class LeaderNotifyBrokerRegisterSucceededHandler extends BaseHandler {
    ...
    public ByteString process(Dto.Frame frame) throws Exception {
        ...
        if (!BrokerService.IS_FIRST_STARTED) {
            BrokerService.initCURRENT_NODE(false);
        }
        BrokerService.IS_FIRST_STARTED = false;
        return null;
    }
}
